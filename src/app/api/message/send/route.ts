...
export async function POST(request: Request) {
  ...
  const updated = await Campus.findOneAndUpdate(
    ...
    [
      {
        $set: {
          name: { $ifNull: ["$name", resolvedCampusName] },
          slug: { $ifNull: ["$slug", campusRecordSlug] },
          address: { $ifNull: ["$address", resolvedAddress] },
          location: { $ifNull: ["$location", resolvedLocation] },
  ...
  const nextAfterInc = Number((updated as any)?.nextSequence || CAMPUS_SEQUENCE_START + 1);
